 ...
 */
public class RecentConsultListActivity extends BaseFloatActivity {
    private RequestQueue mRequestQueue;
    private int start;
    private int pageSize;
    private HotRecConsultAdapter consultAdapter;
    private List<RecentConsult> mList;
    ...
    protected void initTitle() {
        ...
        myTitle.updateCenterTitle("最近咨询");
    }

    protected void initView() {

        mList = new ArrayList<>();
        mList.addAll(DataSupport.findAll(RecentConsult.class));
        PullToRefreshListView recListView = (PullToRefreshListView) findViewById(R.id.hot_consult_list_view);
        consultAdapter = new HotRecConsultAdapter(this, HotRecConsultAdapter.RECENT);
        recListView.setAdapter(consultAdapter);
        consultAdapter.setRecList(mList);

        mHandler = new MyHandler(recListView);
        mHandler.sendEmptyMessageAtTime(0, 1800);

        recListView.setOnRefreshListener(new PullToRefreshBase.OnRefreshListener2<ListView>() {
            @Override
            public void onPullDownToRefresh(PullToRefreshBase<ListView> refreshView) {
                start = 1;
                mList.clear();
                mRequestQueue.add(recentConsultRequest(start, pageSize));
            }
        ...
        recListView.getRefreshableView().setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                RecentConsult consult = (RecentConsult) adapter.getItem(position - 1);
                Intent intent = new Intent(RecentConsultListActivity.this, HRConsultDetailActivity.class);
...
//        Log.e("TAG", ">>>>recent>>>>>>>>.url>>>>>>>>>>>>>" + BaseURLUtil.doGetRecentConsult(start, pageSize));
        return new StringRequest(BaseURLUtil.doGetRecentConsult(start, pageSize), new Response.Listener<String>() {
            ...
            public void onErrorResponse(VolleyError volleyError) {
                Toast.makeText(RecentConsultListActivity.this, volleyError.getMessage(), Toast.LENGTH_SHORT).show();
                consultAdapter.setRecList(mList);
                mHandler.sendEmptyMessageAtTime(0, 1800);
            }
        });
